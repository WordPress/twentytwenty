...
if ( ! function_exists( 'twentytwenty_color_css_frontend' ) ) {
	...
	function add_something() {
		...
		array_walk( $colors, function( $mod, $color ) { 
			...
			return $mod = sanitize_hex_color( maybe_hash_hex_color( get_theme_mod( $mod, $color ) ) );
		} );

		printf( 
			'<style id="twentytwenty-style-inline-css" data-twentytwenty-colors="%1$s">%2$s</style>',
			htmlspecialchars( wp_json_encode( $colors ) ),
			twentytwenty_get_customizer_css( 'front-end' )
		);
	}

	add_action( 'wp_head', 'add_something' );
...
if ( ! function_exists( 'twentytwenty_color_css_frontend' ) ) {
	...
	function twentytwenty_color_css_frontend() {
		echo twentytwenty_get_customizer_css( 'front-end' );
	}
}
