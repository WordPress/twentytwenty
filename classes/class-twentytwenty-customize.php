		...
		public static function register( $wp_customize ) {
			...
			*/
			...
			$wp_customize->add_setting(
				...
				array(
					'default'           => self::get_colors( 'cover_template_overlay_background_color' ),
					'transport'         => 'postMessage',
					'sanitize_callback' => 'sanitize_hex_color',
				)
			);
			...
			$wp_customize->add_setting(
				...
				array(
					'default'           => self::get_colors( 'cover_template_overlay_text_color' ),
					'transport'         => 'postMessage',
					'sanitize_callback' => 'sanitize_hex_color',
				)
			);
			 ...
			 */
			$wp_customize->selective_refresh->add_partial( 'twentytwenty_colors', array(
				'selector'        => '#twentytwenty-style-inline-css',
				'settings'        => array_keys( self::get_colors() ),
				'render_callback' => 'twentytwenty_color_css_frontend',
			) );

		}

		/**
		 * A list of colors theme mods and their defaults.
		 *
		 * @param string $color Theme mod name.
		 */
		public static function get_colors( $color = null ) {
			$main_colors = array_map( function( $mod ) {
				$key = key( $mod );
				$key = $mod['default'];
			...
			$colors = array_merge( $main_colors, array(
				'background_color'                        => '#f5efe0', 
				'cover_template_overlay_background_color' => get_theme_mod( 'accent_color', $main_colors['accent_color'] ), 
				'cover_template_overlay_text_color'       => '#ffffff',
			) );
...
if ( ! function_exists( 'twentytwenty_customize_preview_style' ) ) {
	/**
	 * Add custom CSS generated by the Customizer by hooking into `wp_head`.
	 * 
	 * This will help us add the data attributes needed for selective refresh.
	 */
	function twentytwenty_customize_preview_style() {
		...
		$colors = TwentyTwenty_Customize::get_colors();

		array_walk( $colors, function( $mod, $color ) { 
			$mod = sanitize_hex_color( maybe_hash_hex_color( get_theme_mod( $mod, $color ) ) );
			
			return $mod;
		} );

		printf( 
			'<style id="twentytwenty-style-inline-css" data-twentytwenty-colors="%1$s">%2$s</style>',
			htmlspecialchars( wp_json_encode( $colors ) ), //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			twentytwenty_get_customizer_css( 'front-end' ) //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		); 
	}

	add_action( 'wp_head', 'twentytwenty_customize_preview_style' );

}

if ( ! function_exists( 'twentytwenty_color_css_frontend' ) ) {
	/**
	 * Render the color's css for the selective refresh partial.
	 */
	function twentytwenty_color_css_frontend() {
		echo twentytwenty_get_customizer_css( 'front-end' ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

if ( ! function_exists( 'twentytwenty_customize_preview_js' ) ) {
	/**
	 * Bind JS handlers to instantly live-preview changes.
	 */
	function twentytwenty_customize_preview_js() {
		$theme_version   = wp_get_theme()->get( 'Version' );
		$js_dependencies = array( 'customize-preview', 'jquery' );

		wp_enqueue_script( 'twentytwenty-customize-preview', get_theme_file_uri( '/assets/js/customize-preview.js' ), $js_dependencies, $theme_version, true );
	}

	add_action( 'customize_preview_init', 'twentytwenty_customize_preview_js' );
